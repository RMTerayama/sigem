...
<body>
    ...
    $(document).ready(function() {
        if ($.fn.select2) {
            console.log("✅ Select2 carregado corretamente!");

            $('.select2').each(function() {
                if (!$(this).data('select2')) { // Evita inicialização duplicada
                    $(this).select2({
                        theme: 'bootstrap-5',
    ...
    function adicionarItem() {
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select class="form-select select2" name="itens_saida[${itemCount}][id]" required>
                        <option value=""></option> <!-- Opção vazia para ativar pesquisa -->
        ...
        // Inicializa o Select2 para o novo elemento após ele ser adicionado ao DOM
        setTimeout(() => {
            if ($.fn.select2) {
                $(newItem).find('.select2').each(function() {
                    if (!$(this).data('select2')) { // Evita duplicação
                        $(this).select2({
                            theme: 'bootstrap-5',
                            placeholder: "Selecione ou digite para buscar...",
                            allowClear: true
                ...
                console.error("❌ Erro: Select2 não está disponível!");
            }
        }, 50);
    ...
    function salvarIdentificacao() {
        ...
        document.getElementById(`identificacao-${itemId}`).innerHTML =
            `<strong>Identificações:</strong> ${identificacoes[itemId].join(', ')}`;
    ...
    function ajustarTamanhoCanvas() {
        ...
        canvas.width = rect.width; // Define a largura interna correta
        canvas.height = rect.height; // Define a altura interna correta
        limparAssinatura(); // Garante que não haja resíduos de assinaturas anteriores
    }

    // Redimensiona o canvas sempre que o modal de assinatura for aberto
    ...
    // Obter coordenadas corretas do mouse/toque
    ...
    async function gerarPDF(termoId) {
        ...
        doc.autoTable({
            startY: 70,
            head: [
                ['Item', 'Modelo', 'Quantidade', 'Identificação']
            ],
